...
public class BattleFragment extends Fragment implements View.OnClickListener {
    ...
    Character player;
    Character enemy;
    ArrayList<BattleEffect> playerEffects;
    ArrayList<BattleEffect> enemyEffects;
    View view;
    ...
    public BattleFragment(Character player, Character enemy) {
        ...
        playerEffects = new ArrayList<BattleEffect>();
        enemyEffects = new ArrayList<BattleEffect>();
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        addBattleText("The " + enemy.getName() + " stares you straight in the eyes. What do you do?");
    ...
    public void useAbility(View view) {
        ...
        if (type.equals("ATTACK")) {
            ...
            playerEffects.add(new BattleEffect(stat, damage, duration));
            stat = stat.substring(0, 1) + stat.substring(1).toLowerCase();
            ...
            switch (damage) {
                case 0:
                    ...
                    break;
                default:
                    enemyEffects.add(new BattleEffect(stat, damage, duration));
                    stat = stat.substring(0, 1) + stat.substring(1).toLowerCase();
    ...
    public void enemyUseAbility(int choice) {
        ...
        int duration = enemy.getAbilities().get(choice).GetDuration();
        String type = enemy.getAbilities().get(choice).GetActionType();
        String stat = enemy.getAbilities().get(choice).GetStatType();
    ...
    public void updateBattleEffects() {
        ...
        for (int i = 0; i < playerEffects.size(); i++) {
            ...
            playerEffects.get(i).decrement();

            if (playerEffects.get(i).getDuration() == 0) {
                if (!print) {
                    print = true;
                    addBattleText(" ");
                }

                playerEffects.get(i).reverse(player);
                String stat = playerEffects.get(i).getStat();
                stat = stat.substring(0, 1) + stat.substring(1).toLowerCase();
                addBattleText("Your " + stat + " is back to normal.");
        ...
        for (int i = 0; i < enemyEffects.size(); i++) {
            ...
            enemyEffects.get(i).decrement();

            if (enemyEffects.get(i).getDuration() == 0) {
                if (!print) {
                    print = true;
                    addBattleText(" ");
                }
                
                enemyEffects.get(i).reverse(enemy);
                String stat = enemyEffects.get(i).getStat();
                stat = stat.substring(0, 1) + stat.substring(1).toLowerCase();
                addBattleText("The " + enemy.getName() + "'s " + stat + " is back to normal.");
